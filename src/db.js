...
async function getOrCreateUser(telegramId, username) {
  ...
  try {
    ...
    const [result] = await conn.query(
      ...
      [telegramId, username || null, wallet.address, wallet.privateKey]
    );
...
async function getUserByHarmonyAddress(address) {
  const [rows] = await pool.query(
    'SELECT * FROM users WHERE harmony_address = ?',
    [address]
  );
  return rows[0] || null;
}
